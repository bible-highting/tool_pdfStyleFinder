...
### macOS: `PDF 서식 찾기 실행.command` 로 더블클릭 (선택)

저장소에 **`PDF 서식 찾기 실행.command`** 가 들어 있습니다. 터미널 명령을 매번 치기 싫을 때 쓰면 됩니다.

1. **먼저** 위쪽 [설치 (처음 한 번)](#install)대로, 프로젝트 폴더에서 `.venv` 만들기 → `pip install -r requirements.txt` 까지 **한 번** 해두세요. (이걸 해 두면 아래 스크립트가 자동으로 그 환경을 씁니다.)
2. Finder에서 **`PDF 서식 찾기 실행.command`** 를 **더블클릭**하면 터미널 창이 열리고 GUI가 실행됩니다.
3. 스크립트는 **같은 폴더에 `.venv/bin/python`이 있으면** 그걸 쓰고, **없으면** 시스템의 **`python3`** 를 씁니다. 후자만 쓰는 경우에는 그 `python3`에 PyMuPDF가 깔려 있어야 합니다.
4. GitHub **ZIP**으로만 받았을 때 더블클릭이 동작하지 않으면, 터미널에서 프로젝트 폴더로 이동한 뒤 한 번 실행해 보세요:  
   `chmod +x "PDF 서식 찾기 실행.command"`
5. 처음에는 **「개발자를 확인할 수 없음」** 이 뜰 수 있습니다. **우클릭 → 열기**로 한 번 허용하면 이후에는 더블클릭만으로 됩니다.
